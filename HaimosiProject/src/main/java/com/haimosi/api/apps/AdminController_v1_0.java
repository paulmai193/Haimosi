...
public class AdminController_v1_0 {
	...
	public String addItem(FormDataMultiPart formData) {
		...
		else if (itemUnit == null || itemUnit.trim().isEmpty()) {
			throw new BadParamException(new Throwable("Invalid item unit"));
		}

		try (InputStream fileStream = fileBody.getValueAs(InputStream.class); ItemDAO itemDAO = AbstractDAO.borrowFromPool(DAOPool.itemPool)) {
			JsonObject jsonResponse = new JsonObject();

			// Create item record
			ItemPOJO item = new ItemPOJO(null, itemName, itemDesc, itemPrice, itemUnit, null, itemAmount);
			...
			if (item != null) {
				...
				try (OutputStream fOut = new FileOutputStream(tempFile)) {
					...
					if (ImageChecker.isImage(tempFile)) {
						...
						String nameItem = System.currentTimeMillis() + fileMineType;
						File fileItem = new File(directoryItem.getAbsolutePath() + Constant.SEPERATOR + nameItem);
						ScaleImage.doScale(tempFile, fileItem, 320);

						// Write avatar success, update user in database
						item.setPhoto(nameItem);
						itemDAO.saveOrUpdate(session, item);
						HibernateUtil.commitTransaction(session);
	...
	@Produces(value = { MediaType.APPLICATION_JSON })
	public String listTransactions(@QueryParam(ParamDefine.PAGE) IndexParam page) {
		if (page == null) {
